...
export function BillRow({ bill, onPress }: { bill: BillWithSplits; onPress: () => void }) {
  ...
  const tone = status === 'overdue' ? 'coral' : status === 'due-soon' ? 'amber' : 'brand';
  ...
  return (
    <Card onPress={onPress}>
      ...
      <View className="mt-3 gap-2 border-t border-sand-100 pt-2.5">
        <ProgressBar ratio={progress.ratio} tone={status === 'settled' ? 'brand' : tone} />
